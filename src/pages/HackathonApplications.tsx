...
import { User, MapPin, Calendar, Check, X, MessageCircle, ArrowLeft, Trophy, Users } from 'lucide-react';
import { api } from '../services/api';
import LoadingSpinner from '../components/LoadingSpinner';
import toast from 'react-hot-toast';

export default function HackathonApplications() {
  const { id } = useParams();
  ...
  useEffect(() => {
    fetchHackathonAndApplications();
  }, [id]);

  const fetchHackathonAndApplications = async () => {
    try {
      const [hackathonResponse, applicationsResponse] = await Promise.all([
        api.get(`/hackathons/${id}`),
        api.get(`/hackathons/${id}/applications`)
      ]);
  ...
  const handleStatusUpdate = async (applicationId: number, status: string) => {
    ...
    try {
      await api.put(`/hackathons/applications/${applicationId}/status`, { status });
      toast.success(`Application ${status} successfully!`);
      fetchHackathonAndApplications(); // Refresh data
    } catch (error: any) {
  ...
  if (!hackathon) {
    return (
      <div className="text-center py-12">
        ...
        <Link to="/hackathons" className="text-blue-600 hover:text-blue-700">
          Back to Team Search
        </Link>
      </div>
    );
      ...
      <div className="bg-white rounded-xl p-8 shadow-sm">
        <div className="flex items-center space-x-4 mb-6">
          ...
          <div>
            ...
            <p className="text-gray-600 mt-1">Manage team applications for {hackathon.hackathon_name}</p>
          </div>
        </div>
        ...
        {applications.length > 0 ? (
              ...
              <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex flex-wrap items-center gap-4 text-sm text-gray-500 mb-4">
                        ...
                        {application.user.experience && (
                          <div className="flex items-center space-x-1">
                            <span className="capitalize">{application.user.experience} level</span>
                          </div>
                        )}
                      </div>
                      ...
                      {application.message && (
                        <div className="bg-gray-50 rounded-lg p-4 mb-4">
                          <h4 className="text-sm font-medium text-gray-700 mb-2">Why they want to join</h4>
                ...
                <div className="flex flex-col space-y-3 lg:ml-8">
                  ...
                  <Link
                    ...
                    className="bg-purple-100 text-purple-700 px-4 py-2 rounded-lg hover:bg-purple-200 transition-colors text-center font-medium flex items-center justify-center space-x-2"
                  >
          ...
          <div className="bg-white rounded-xl p-12 shadow-sm text-center">
            ...
            <p className="text-gray-600">
              When developers apply to join your team, their applications will appear here.
            </p>
          </div>
        )}
      </div>
